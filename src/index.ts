...
import { convertToolsToGLM, extractToolCalls, formatToolResults, STANDARD_TOOLS } from './tools'
import {
  extractTextContent,
  extractAndFormatToolCalls,
  calculateCost,
  hasToolCalls,
} from './parser'
import type { SdkOptions, SDKMessage, GLMMessage, ToolResult } from './types'
 ...
 */
export async function* query({ prompt, options = {} }: QueryOptions): AsyncGenerator<SDKMessage> {
  ...
  try {
    const lastResponse = await apiClient.createChatCompletion(
