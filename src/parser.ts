...
import { extractToolCalls } from './tools'
 ...
 */
export function extractTextContent(response: GLMResponse): string {
 ...
 */
export function hasToolCalls(response: GLMResponse): boolean {
  if (!response.content || response.content.length === 0) {
    return false
  }
 ...
 */
export function extractAndFormatToolCalls(response: GLMResponse) {
  if (!response.content || response.content.length === 0) {
    return []
  }
 ...
 */
export function generateToolResultMessage(toolName: string, result: string): string {
